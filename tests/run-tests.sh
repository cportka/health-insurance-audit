#!/usr/bin/env bash
#
# run-tests.sh — Portka standard suite for health-insurance-audit.
# Binds to the repo's version source of truth (package.json), checks it is SemVer and that
# CHANGELOG.md and the README **Version:** line agree, runs the Node test suite (node --test),
# then runs any tests/cases/*.sh. Exit 0 if nothing FAILed, 1 otherwise.
#
set -uo pipefail
...
if [[ -z "$SRC_VER" ]]; then
  fail "no version source found (package.json / VERSION / README **Version:**)"
else
...
# Node test suite (node:test). Run the whole tests/ tree; report a single pass/fail.
if command -v node >/dev/null 2>&1; then
  if node --test 2>&1 | tail -40; then
    pass "node --test suite"
  else
    fail "node --test suite"
  fi
else
  printf '  \033[33mSKIP\033[0m  node --test (node not found)\n'
fi
